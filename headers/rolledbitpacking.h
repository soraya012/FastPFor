            ...
            if (inword > 32 - bit) {
                *out++ = __bitmask<bit, mask> (
                        (in[pointer / 32] >> inword) | (in[pointer / 32 + 1]
                                << (32 - inword)));
                if (++counter == length)
                    return reinterpret_cast<const uint8_t *> (&in[pointer / 32
                            + 1]) + 1 + byteoffset(bit + 32 - inword - 1);
            } else {
...
void __unpack(const uint32_t * __restrict__ in, uint32_t * __restrict__ out) {
    ...
    for (uint32_t k = 0; k < bit; ++k, ++in) {
        // did not unroll; compiler did not detect invariant that inwordpointer is always
        // small positive number
        /* for (; inwordpointer   /\*+   bit <= 32 *\/ <= 32-bit; inwordpointer += bit) { */
...
void unpack_tight(const uint32_t * __restrict__ in,
